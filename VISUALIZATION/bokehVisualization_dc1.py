...
from bokeh.palettes import Turbo256, Viridis256, Spectral8, Purples256, Blues256, Greens256, Oranges256, Blues3
    ...
    # try:
        ...
        if createViz(mln_User, final_output_cluster_name):
            ...
            # if therr are edges
                ...
                size_by_this_attribute = 'adjusted_node_size'
                color_by_this_attribute = Blues3[0]
                # color_by_this_attribute = 'modularity_color'

                # Title of the graph -----------------------------------------------------------------
                title = f"{clusterName} Network Graph using Degree Centrality"
                # Hovering over the nodes ------------------------------------------------------------
                HOVER_TOOLTIPS = [
                    ("Node Label", "@label"),
                    ("Connections", "@degree"),
                ...
                # show(plot)
                # SAVE FIGURE ------------------------------------------------------------------------
                save(plot, os.path.join(endPath, "visualization",f"bokeh_DC_{clusterName}_Network.html"), title=f"{clusterName} Network Graph using Degree Centrality", resources='inline')

                return os.path.join(mln_User, "visualization",f"bokeh_DC_{clusterName}_Network.html")
        else:
